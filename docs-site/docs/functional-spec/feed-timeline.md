...
Feature: フィードタイムライン
  ...
  Scenario: タグが選択されているときは、そのタグに紐づくフィードのみ新着順で表示される
    ...
    When タイムラインを開く
    Then タイムラインには選択されたタグに基づくフィードのみ新着順で表示される
  ...
  Scenario: タグが選択されていないときは、タグ設定で設定した全タグに紐づくフィードが新着順で表示される
    ...
    When タイムラインを開く
    Then タイムラインにはタグ設定で設定した全タグに基づくフィードが新着順で表示される

  Scenario: タグが設定されていないときは、タグ設定ページにリダイレクトする
    Given ユーザーがログインしている
      And ユーザーがタグ設定でいずれのタグも設定していない
    When ユーザーがタイムラインを開く
    Then タグ設定ページにリダイレクトする
